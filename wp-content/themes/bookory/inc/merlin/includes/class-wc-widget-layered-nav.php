...
class Bookory_Widget_Layered_Nav extends WC_Widget {

    /**
     * Constructor.
     */
    public function __construct() {
        $this->widget_cssclass    = 'woocommerce bookory_widget_layered_nav woocommerce-widget-layered-nav';
        $this->widget_description = esc_html__('Display a list of attributes to filter products in your store.(Bookory custom)', 'bookory');
        $this->widget_id          = 'bookory_woocommerce_layered_nav';
        $this->widget_name        = esc_html__('Bookory Filter Products by Attribute', 'bookory');
        parent::__construct();
    }
    ...
    public function init_settings() {
        ...
        $this->settings = array(
            'title'      => array(
                'type'  => 'text',
                'std'   => esc_html__('Filter by', 'bookory'),
                'label' => esc_html__('Title', 'bookory'),
            ),
            'attribute'  => array(
                'type'    => 'select',
                'std'     => '',
                'label'   => esc_html__('Attribute', 'bookory'),
                'options' => $attribute_array,
            ),
            'query_type' => array(
                'type'    => 'select',
                'std'     => 'and',
                'label'   => esc_html__('Query type', 'bookory'),
                'options' => array(
                    'and' => esc_html__('AND', 'bookory'),
                    'or'  => esc_html__('OR', 'bookory'),
                ),
            ),
        );
    }
    ...
    protected function layered_nav_list($terms, $taxonomy, $query_type) {
        ...
        foreach ($terms as $term) {
            ...
            if ($count > 0 || $option_is_set) {
                ...
                $term_html = '<a rel="nofollow" class="bookory-button-type" href="' . esc_url($link) . '">' . esc_html($term->name) . '</a>';
                    ...
                    if (isset($fields[0]["type"])) {
                        ...
                        switch ($fields[0]["type"]) {
                            case 'color':
                                ...
                                $term_html = '';
                                $term_html .= '<a rel="nofollow" class="bookory-color-type" href="' . esc_url($link) . '">';
                            ...
                            case 'image':
                                ...
                                $term_html = '';
                                $term_html .= '<a rel="nofollow" class="bookory-image-type" href="' . esc_url($link) . '">';
