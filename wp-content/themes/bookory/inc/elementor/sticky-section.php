...
class Bookory_Sticky_Module {
	...
	public function scripts(){
        ...
        wp_register_script('elementor-sticky', get_template_directory_uri() . '/assets/js/vendor/jquery.sticky.js', ['jquery'], $bookory_version, true);
        wp_enqueue_script('bookory-elementor-sticky', get_template_directory_uri() . '/assets/js/vendor/sticky.js', ['jquery', 'elementor-frontend', 'elementor-sticky'], $bookory_version, true);
    }
    ...
    public function register_controls( Element_Base $element ) {
        $element->add_control(
            'sticky',
            [
                'label' => esc_html__( 'Sticky', 'bookory' ),
                'type' => Controls_Manager::SELECT,
                'options' => [
                    '' => esc_html__( 'None', 'bookory' ),
                    'top' => esc_html__( 'Top', 'bookory' ),
                    'bottom' => esc_html__( 'Bottom', 'bookory' ),
                ],
                'separator' => 'before',
                'render_type' => 'none',
        ...
        foreach ( $active_devices as $device ) {
            $label = 'desktop' === $device ? esc_html__( 'Desktop', 'bookory' ) : $active_breakpoint_instances[ $device ]->get_label();
        ...
        $element->add_control(
            'sticky_on',
            [
                'label' => esc_html__( 'Sticky On', 'bookory' ),
        ...
        $element->add_responsive_control(
            'sticky_offset',
            [
                'label' => esc_html__( 'Offset', 'bookory' ),
        ...
        $element->add_responsive_control(
            ...
            [
                'label' => esc_html__( 'Effects Offset', 'bookory' ),
                'type' => Controls_Manager::NUMBER,
                'default' => 0,
                'min' => 0,
                'max' => 1000,
        ...
        if ( $this->is_instance_of( $element, $types ) ) {
            ...
            $element->add_control(
                'sticky_parent',
                [
                    'label' => esc_html__( 'Stay In Column', 'bookory' ),
